...
public class SolrCoreCheckLockOnStartupTest extends SolrTestCaseJ4 {
  ...
  public void setUp() throws Exception {
    ...
    Files.deleteIfExists(
        Path.of(Path.of(initAndGetDataDir().toString(), "index").toString(), IndexWriter.WRITE_LOCK_NAME));
  }
  ...
  public void testSimpleLockErrorOnStartup() throws Exception {
    ...
    Directory directory =
        newFSDirectory(
            Path.of(initAndGetDataDir().toString(), "index"), SimpleFSLockFactory.INSTANCE);
  ...
  public void testNativeLockErrorOnStartup() throws Exception {
    ...
    Path indexDir = Path.of(initAndGetDataDir().toString(), "index");
    if (log.isInfoEnabled()) {
      log.info("Acquiring lock on {}", indexDir.toAbsolutePath().toString());
    }
