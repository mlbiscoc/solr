...
/**
 * Testing of pivot facets on multiple collections. This class mainly aims to test that there is no
 * issue with stop words on multiple collections. Facets pivot counts are validated elsewhere.
 * There's no validation of counts here.
 */
@SuppressSSL
public class FacetPivot2CollectionsTest extends SolrCloudTestCase {
  ...
  public static void setupCluster() throws Exception {
    ...
    CollectionAdminResponse response =
        CollectionAdminRequest.createAlias(ALIAS, COLL_A + "," + COLL_B)
            .process(cluster.getSolrClient());
  ...
  public void testOneCollectionPivotName() throws SolrServerException, IOException {
    SolrParams params =
        params(
            ...
            "true",
            "facet.field",
            SUBJECT_TXT_FIELD_MULTIVALUED,
            "facet.pivot",
            NAME_TXT_FIELD_NOT_MULTIVALUED);
  ...
  public void testOneCollectionPivotInt() throws SolrServerException, IOException {
    SolrParams params =
        params(
            ...
            SUBJECT_TXT_FIELD_MULTIVALUED,
            "facet.pivot",
            DYNAMIC_INT_FIELD_NOT_MULTIVALUED);
    QueryResponse response = cluster.getSolrClient().query(COLL_A, params);
    NamedList<Object> result = response.getResponse();
    if (result.get("facet_counts") == null) {
      fail("Facet pivot on one collection failed");
  ...
  public void testOneCollectionPivotFloat() throws SolrServerException, IOException {
    SolrParams params =
        params(
            ...
            SUBJECT_TXT_FIELD_MULTIVALUED,
            "facet.pivot",
            DYNAMIC_FLOAT_FIELD_NOT_MULTIVALUED);
    QueryResponse response = cluster.getSolrClient().query(COLL_A, params);
    NamedList<Object> result = response.getResponse();
    if (result.get("facet_counts") == null) {
      fail("Facet pivot on one collection failed");
  ...
  public void testOneCollectionPivotDate() throws SolrServerException, IOException {
    SolrParams params =
        params(
            ...
            SUBJECT_TXT_FIELD_MULTIVALUED,
            "facet.pivot",
            DYNAMIC_DATE_FIELD_NOT_MULTIVALUED);
    QueryResponse response = cluster.getSolrClient().query(COLL_A, params);
    NamedList<Object> result = response.getResponse();
    if (result.get("facet_counts") == null) {
      fail("Facet pivot on one collection failed");
  ...
  public void testOneCollectionPivotTitleFileType() throws SolrServerException, IOException {
    SolrParams params =
        params(
            ...
            "true",
            "facet.field",
            TITLE_TXT_FIELD_NOT_MULTIVALUED,
            "facet.pivot",
            String.join(",", FILETYPE_TXT_FIELD_NOT_MULTIVALUED, TITLE_TXT_FIELD_NOT_MULTIVALUED));
    QueryResponse response = cluster.getSolrClient().query(COLL_A, params);
    NamedList<Object> result = response.getResponse();
    if (result.get("facet_counts") == null) {
      fail("Facet pivot on one collection failed");
  ...
  public void testAliasPivotType() throws SolrServerException, IOException {
    SolrParams params =
        params(
            ...
            SUBJECT_TXT_FIELD_MULTIVALUED,
            "facet.pivot",
            FILETYPE_TXT_FIELD_NOT_MULTIVALUED);
    final QueryResponse response = cluster.getSolrClient().query(ALIAS, params);
    NamedList<Object> result = response.getResponse();
    if (result.get("facet_counts") == null) {
      fail("Facet pivot on the alias failed");
  ...
  private static void index(final String collection, final int numDocs)
    ...
    for (int i = 0; i < numDocs; i++) {
      ...
      solrDoc.addField(
          DYNAMIC_DATE_FIELD_NOT_MULTIVALUED, skewed(randomSkewedDate(), randomDate()));
      solrDoc.addField(
          DYNAMIC_INT_FIELD_NOT_MULTIVALUED,
          skewed(TestUtil.nextInt(random(), 0, 100), random().nextInt()));
      solrDoc.addField(
          DYNAMIC_FLOAT_FIELD_NOT_MULTIVALUED,
          skewed(1.0F / random().nextInt(25), random().nextFloat() * random().nextInt()));
  ...
  private static Map<String, SolrInputField> addDocFields(final int id) {
    ...
    textField2.setValue(
        new String[] {randomText(random, 25, false), randomText(random, 25, false)});
  ...
  private static String randomText(
      final Random random, final int maxWords, final boolean addNonAlphaChars) {
