...
public class AuditLoggerIntegrationTest extends SolrCloudAuthTestCase {
  ...
  public void testAsyncQueueDrain() throws Exception {
    ...
    final List<AuditEvent> events =
        new ArrayList<>(harness.receiver.waitForAuditEvents(preShutdownEventsAllowed));
    ...
    final Thread shutdownThread =
        new SolrNamedThreadFactory("shutdown")
            .newThread(
                () -> {
                  try {
  ...
  public void testMuteAdminListCollections() throws Exception {
    setupCluster(
        false, null, false, "\"type:UNKNOWN\"", "[ \"path:/admin\", \"param:action=LIST\" ]");
    runThreeTestAdminCommands();
  ...
  public void authValid() throws Exception {
    ...
    { // valid READ requests: #1 with, and #2 without, (valid) Authentication
      ...
      assertAuditEvent(
          events.get(0), COMPLETED, "/admin/collections", ADMIN, null, 200, "action", "LIST");
      assertAuditEvent(
          events.get(1), COMPLETED, "/admin/collections", ADMIN, "solr", 200, "action", "LIST");
    }
    ...
    { // valid CREATE request: Authenticated admin user should be allowed to CREATE collection
      ...
      assertAuditEvent(
          events.get(1), COMPLETED, "/admin/collections", ADMIN, null, 200, "action", "CREATE");
    }
  }
  ...
  public void authFailures() throws Exception {
    ...
    { // invalid request: Authenticated user not allowed to CREATE w/o Authorization
      final SolrException e =
          expectThrows(
              SolrException.class,
              () -> {
                final Create createRequest =
                    CollectionAdminRequest.createCollection("test_jimbo", 1, 1);
    ...
    { // invalid request: Anon user not allowed to CREATE w/o authentication + authorization
      ...
      final List<AuditEvent> events = receiver.waitForAuditEvents(1);
      assertAuditEvent(
          events.get(0), REJECTED, "/admin/collections", ADMIN, null, 401, "name", "test_anon");
    }
          ...
          expectThrows(
              ...
              () -> {
                Create createRequest =
                    CollectionAdminRequest.createCollection("test_wrongpass", 1, 1);
  ...
  private static void assertAuditEvent(
      AuditEvent e, EventType type, String path, String... params) {
  ...
  private static String AUTH_SECTION =
      ",\n"
          ...
          + "    \"credentials\":{\"solr\":\""
          + getSaltedHashedValue(SOLR_PASS)
          + "\","
          + "                     \"jimbo\":\""
          + getSaltedHashedValue(JIMBO_PASS)
          + "\"}},\n"
          + "  \"authorization\":{\n"
  ...
  /**
   * Starts the cluster with a security.json built from template, using CallbackAuditLoggerPlugin.
   * The params to this method will fill the template.
   *
  ...
  private void setupCluster(
    ...
    securityJson =
        securityJson.replace(
            "_SEMAPHORE_", null == semaphoreName ? "null" : "\"" + semaphoreName + "\"");
   ...
   */
  // we don't really care about the InterruptedException that could be thrown from close in test
