 ...
 *
 * <p><b>NOTE:</b> This test sets up a static instance of MiniSolrCloud with a single collection and
 * several clients pointed at specific nodes. These are all re-used across multiple test methods,
...
public class TestTolerantUpdateProcessorCloud extends SolrCloudTestCase {
  ...
  public static void createMiniSolrCloudCluster() throws Exception {
    ...
    final Path configDir =
            Path.of(TEST_HOME() + File.separator + "collection1" + File.separator + "conf");
      ...
      for (UpdateRequest r :
        ...
        e =
            expectThrows(
                ...
                () -> r.process(c));
        assertEquals(
            "not the expected opportunistic concurrency failure code: "
                + r.toString()
                + " => "
                + e.getMessage(),
            409,
  ...
  protected static void testVariousDeletes(SolrClient client) throws Exception {
    ...
    // attempt to delete individual doc id(s) that should fail because of opportunistic concurrency
    // constraints
    ...
    assertUpdateTolerantErrors(
        "failed opportunistic concurrent delete by id for 2 bogus docs",
        rsp,
        delIErr(S_ONE_PRE + "X"),
        delIErr(S_ONE_PRE + "Y"));
    ...
    assertEquals(0, rsp.getStatus());
    assertUpdateTolerantErrors(
        "failed opportunistic concurrent delete by id for 2 docs",
        rsp,
        delIErr(docId1),
        delIErr(docId2));
    assertQueryDocIds(client, true, docId1, docId2);
    ...
    assertEquals(0, rsp.getStatus());
    assertUpdateTolerantErrors(
        "failed opportunistic concurrent delete by query", rsp, delQErr("bogus_field:foo"));
    assertQueryDocIds(client, true, docId1, docId2);
    ...
    assertEquals(0, rsp.getStatus());
    assertUpdateTolerantErrors(
        "failed opportunistic concurrent delete by id: exists", rsp, delIErr(docId1));
    assertQueryDocIds(client, true, docId1, docId2);
    ...
    assertEquals(0, rsp.getStatus());
    assertUpdateTolerantErrors(
        "failed opportunistic concurrent delete by id: bogus", rsp, delIErr("bogusId"));
  ...
  protected static void testVariousAdds(SolrClient client) throws Exception {
    ...
    rsp =
        update(
                params(
                    "update.chain", "tolerant-chain-max-errors-not-set",
                    "commit", "true"),
    ...
    assertUpdateTolerantAddErrors(
        "many docs, 1 from each shard should fail", rsp, S_ONE_PRE + "15", S_TWO_PRE + "22");
    ...
    assertEquals(0, rsp.getStatus());
    assertUpdateTolerantAddErrors(
        "many docs, 1 from each shard (+ no id) should fail",
        rsp,
        S_ONE_PRE + "15",
        "(unknown)",
        S_TWO_PRE + "22");
                    ...
                    .process(client));
    {
      ...
      for (ToleratedUpdateError err : actualKnownErrs) {
        ...
        assertTrue(
            "failed err msg didn't match expected value: " + err,
    ...
    assertEquals(0, client.commit().getStatus()); // need to force since update didn't finish
    assertQueryDocIds(
        client, true, S_ONE_PRE + "z", S_ONE_PRE + "y", S_TWO_PRE + "z", S_TWO_PRE + "y" // first
  ...
  protected static void testAddsMixedWithDeletes(SolrClient client) throws Exception {
    ...
    assertEquals(0, rsp.getStatus());
    assertUpdateTolerantErrors(
        "shard2 add fail, shard1 delI fail",
        rsp,
  ...
  public static void assertUpdateTolerantErrors(
      String assertionMsgPrefix, UpdateResponse response, ExpectedErr... expectedErrs) {
  ...
  /**
   * Asserts that the specified document ids do/do-not exist in the index, using both the specified
   * client, and the CLOUD_CLIENT
   */
  public static void assertQueryDocIds(SolrClient client, boolean shouldExist, String... ids)
      throws Exception {
    for (String id : ids) {
      assertEquals(
