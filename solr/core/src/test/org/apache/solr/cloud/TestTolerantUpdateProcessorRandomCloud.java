...
/**
 * Test of TolerantUpdateProcessor using a randomized MiniSolrCloud. Reuses some utility methods in
 * {@link TestTolerantUpdateProcessorCloud}
 *
...
public class TestTolerantUpdateProcessorRandomCloud extends SolrCloudTestCase {
  ...
  public static void createMiniSolrCloudCluster() throws Exception {
    ...
    log.info(
        "Configuring cluster: servers={}, shards={}, repfactor={}",
        numServers,
        numShards,
        repFactor);
      ...
      for (int cmdIter = 0; cmdIter < numCmds; cmdIter++) {
        ...
        if (random().nextBoolean()) {
          ...
          if (causeError && (0 == TestUtil.nextInt(random(), 0, 21))) {
            ...
            final String val =
                causeError ? "bogus_val" : ("" + TestUtil.nextInt(random(), 42, 666));
  ...
  public void testSanityRandomUnsetBit() {
    ...
    assertEquals(
        "wrong nextBit at redundant end of all iters", -1, randomUnsetBit(random(), bits, max));
  }
