 ...
 *
 * <p>This implementation (with help from subclass {@link OverseerTaskQueue}) is used for the <code>
 * /overseer/collection-queue-work</code> queue used for Collection and Config Set API calls to the
 * Overseer.
 *
 * <p><i>Implementation note:</i> In order to enqueue a message into this queue, a {@link
 * CreateMode#EPHEMERAL_SEQUENTIAL} response node is created and watched at <code>
 * /overseer/collection-queue-work/qnr-<i>monotonically_increasng_id</i></code>, then a
 * corresponding {@link CreateMode#PERSISTENT} request node reusing the same id is created at <code>
 * /overseer/collection-queue-work/qn-<i>response_id</i></code>.
 */
public class ZkDistributedQueue implements DistributedQueue {
   ...
   * Theory of operation:
   *
   * <p>Under ordinary circumstances we neither watch nor poll for children in ZK. Instead we keep
   * an in-memory list of known child names. When the in-memory list is exhausted, we then fetch
   * from ZK.
   *
   * <p>We only bother setting a child watcher when the queue has no children in ZK.
   */
   ...
   *
   * <p>Package-private to support {@link OverseerTaskQueue} specifically.
   */
  @Override
  public Collection<Pair<String, byte[]>> peekElements(
      int max, long waitMillis, Predicate<String> acceptFilter)
      throws KeeperException, InterruptedException {
