...
public abstract class AuditLoggerPlugin implements Closeable, Runnable, SolrInfoBean {
  ...
  /**
   * Initialize the plugin from security.json. This method removes parameters from config object
   * after consuming, so subclasses can check for config errors.
   *
  ...
  public void init(Map<String, Object> pluginConfig) {
    ...
    blockAsync =
        Boolean.parseBoolean(String.valueOf(pluginConfig.getOrDefault(PARAM_BLOCKASYNC, false)));
    blockingQueueSize =
        async
            ? Integer.parseInt(
                String.valueOf(pluginConfig.getOrDefault(PARAM_QUEUE_SIZE, DEFAULT_QUEUE_SIZE)))
            : 1;
    int numThreads =
        async
            ? Integer.parseInt(
                String.valueOf(pluginConfig.getOrDefault(PARAM_NUM_THREADS, DEFAULT_NUM_THREADS)))
            : 1;
  ...
  /**
   * Called by the framework, and takes care of metrics tracking and to dispatch to either
  ...
  /**
   * Returns true if any of the configured mute rules matches. The inner lists are ORed, while rules
   * inside inner lists are ANDed
   *
   * @param event the audit event
   */
  ...
  /**
   * Enqueues an {@link AuditEvent} to a queue and returns immediately. A background thread will
   * pull events from this queue and call {@link #audit(AuditEvent)}
   *
   * @param event the audit event
   */
    ...
    public String formatEvent(AuditEvent event) {
      try {
        ...
        throw new SolrException(
            SolrException.ErrorCode.SERVER_ERROR, "Error converting Event to JSON", e);
      }
    }
  }

  /**
   * Waits 30s for async queue to drain, then closes executor threads. Subclasses should either call
   * <code>super.close()</code> or {@link #waitForQueueToDrain(int)} <b>before</b> shutting itself
   * down to make sure they can complete logging events in the queue.
   */
  @Override
      ...
      while ((!queue.isEmpty() || auditsInFlight.get() > 0) && !timeOut.hasTimedOut()) {
        try {
          if (log.isInfoEnabled()) {
            log.info(
                "Async auditlogger queue still has {} elements and {} audits in-flight, sleeping to drain...",
                queue.size(),
                auditsInFlight.get());
          }
          timeOut.sleep(1000);
      ...
      if (o != null) {
        if (o instanceof List) {
          ...
          throw new SolrException(
              ...
              "The " + PARAM_MUTE_RULES + " configuration must be a list");
        }
      }
    }
    ...
    private MuteRule parseRule(Object ruleObj) {
      try {
        ...
        if (rule.startsWith("type:")) {
          AuditEvent.RequestType muteType =
              AuditEvent.RequestType.valueOf(rule.substring("type:".length()));
          return event -> event.getRequestType() != null && event.getRequestType().equals(muteType);
        }
        if (rule.startsWith("collection:")) {
          return event ->
              event.getCollections() != null
                  && event.getCollections().contains(rule.substring("collection:".length()));
        }
        if (rule.startsWith("user:")) {
          return event ->
              event.getUsername() != null
                  && event.getUsername().equals(rule.substring("user:".length()));
        }
        if (rule.startsWith("path:")) {
          return event ->
              event.getResource() != null
                  && event.getResource().startsWith(rule.substring("path:".length()));
        }
        if (rule.startsWith("ip:")) {
          return event ->
              event.getClientIp() != null
                  && event.getClientIp().equals(rule.substring("ip:".length()));
        }
        if (rule.startsWith("param:")) {
          String[] kv = rule.substring("param:".length()).split("=");
          if (kv.length == 2) {
            return event ->
                event.getSolrParams() != null && kv[1].equals(event.getSolrParamAsString(kv[0]));
          } else {
            throw new SolrException(
                SolrException.ErrorCode.SERVER_ERROR,
                "The 'param' muteRule must be of format 'param:key=value', got " + rule);
          }
        }
        ...
        throw new SolrException(
            SolrException.ErrorCode.SERVER_ERROR,
            "There was a problem parsing muteRules. Must be a list of valid rule strings",
            e);
      }
    }

    /**
     * Returns true if any of the configured mute rules matches. The inner lists are ORed, while
     * rules inside inner lists are ANDed
     */
